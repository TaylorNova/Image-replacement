...
inp.addParameter('ShowProc', true);
inp.parse(varargin{:});
...
if inp.Results.ShowProc
    figure,imshow(im,[]),set(gcf,'name','原图');
    figure,imshow(g{1},[]),set(gcf,'name','垂直方向梯度图');
    figure,imshow(g{2},[]),set(gcf,'name','水平方向梯度图');
end
% non-maximum suppression
[e, gMag] = nonmaxSuppress(g, 'Method', inp.Results.SMethod, ...
    'Radius', inp.Results.SRadius, 'SubPixel', inp.Results.SubPixel);
if inp.Results.ShowProc
    figure,imshow(e.*gMag,[]),set(gcf,'name','非极大值抑制后边缘');
    figure,imshow(gMag,[]),set(gcf,'name','幅度图');
end
...
if ~strcmp(tmeth, 'none')
    ...
    if inp.Results.SubPixel
        ...
        e = hystThresh(e, gMag, thresh, inp.Results.ShowProc,tconn);
    end
end
